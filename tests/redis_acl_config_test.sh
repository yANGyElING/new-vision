...
expected_nodeapp='on ">$$NV_REDIS_NODEAPP_PASSWORD" ~nv:nodeapp:v1:* +ping +hgetall +hset +del +set +multi +exec +sadd +smembers +srem -flushdb -flushall'
expected_access='on ">$$NV_REDIS_ACCESS_PASSWORD" ~nv:access:v1:* +ping +get +set +del +hget +hgetall +hset +hdel +incr +expire +xadd +xrange +xtrim +multi +exec +discard +watch +unwatch +sadd +smembers +srem -flushdb -flushall'
