...
**进度**：C 模块在聚合一个批次（按 SN）期间，每收到一帧即向事件流发 `catalog.progress` `{device_id, sn, received, total}`；终态发 `catalog.result` `{ok, count, error?}`。`total` 取最近一帧的 SumNum（协议允许前后期报数不一致，以最晚的一帧为准）。前端进度条 = `received / total`。

**滑动超时**：
- **首包超时 30s**：查询发出后 30s 内未收到任何一帧 → 该次同步失败；
- **滑动超时 10s**：收到首包后，超时重置为 10s，每再来一帧重新计 10s；任一 10s 窗口无新帧 → 该次同步失败。
- 失败时整批作废、不写库（D46 不变）；事件里带上 `error` 及已收帧数，页面可显示「收 26/32 后超时」。

**SSE 事件类型**：`catalog.progress` / `catalog.result`；断线重连后由客户端主动查询该设备当前同步状态兑底。

## 7B. 管理页同步进度展示（D51）

同步过程与状态在设备行内用**圆环 + 数字**表达，遵守 Apple 设计语言（.emma/skills/apple-design）：

- **圆环**：管理页 16px、用户页横幅 24px；线宽 2.5；12 点钟起弧、圆头线帽；进行中蓝色、成功变绿、失败停在最后位置变红。完成时弧收拢为满环 + ✓ 轻微回弹浮现（450ms，cubic-bezier(0.34,1.2,0.28,1)），不跳变。
- **数字格式**：`百分比 · 实收/预计路数`（如 `62% · 20/32 路`）；数字段固定宽度右对齐 + tabular-nums，位数变化不抖动。**不展示任何机制文案**（首包、超时重置等进抽屉/日志）。
- **四态行内呈现**：✓ 时间·路数 / 圆环+数字 / ✗ 红环停位+超时时间 / —（从未同步，无文案无引导按钮，统一用「↻ 刷新目录」）。
- **信息密度削减**：设备名下方不放告警副标题（异常由通道列表达）；所属组织只显直接父级（完整路径进 title tooltip）；通道列 = 点阵 + `在线数/总数 在线`；「注册于 N 分钟前」收进在线圆点 hover tooltip。
- **组织树**：嵌套容器 + 左侧 1px 导轨线（VSCode/Notion 式）；父节点行首 ▸ 折叠箭头（可收起），叶子节点小圆点；不用肘部连接线。
- **基础语言**：按钮 :active 缩放 0.97/100ms；毛玻璃卡片 blur 12 + saturate 180%；大标 28px/-0.01em、正文 15px、辅助 12–13px；自动暗色；prefers-reduced-motion 全部位移动画退化为透明度。
- **离线设备的通道列**：保留最后成功同步的快照（点阵 + 计数灰显），不实时推断离线设备的通道状态。
...
- **D50 同步进度与滑动超时**：SumNum 驱动进度条（管理页 + 用户页横幅均显示）；首包 30s、收到首包后滑动 10s；权限点 `catalog:refresh`（用户页面也可触发、仅提示不禁用，出错文案用户化）。
- **D51 同步进度展示规范**：圆环（无长条进度）；`百分比 · 实收/预计` 数字格式且防抖动；无机制文案；未同步态显示「—」统一用刷新按钮；组织树嵌套导轨式；Apple 设计语言全量约束（:active 缩放、毛玻璃、暗色、减动效）。
