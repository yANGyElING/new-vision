// Package siptest provides a test-only SIP client used by the console to
// exercise the node-access (Kamailio) control plane: REGISTER with Digest
// authentication, GB28181 KeepAlive MESSAGE, and unregister. Behavior is
// unchanged from the pre-refactor nodeapp package.
package siptest
...
// SIPSimulator is a test-only SIP client.
...
func (s *SIPSimulator) Register(ctx context.Context, accessID string, expires int) (SIPTestResult, error) {
	d, err := s.lookup.GetByAccessID(ctx, accessID)
	if err != nil {
		return SIPTestResult{}, err
	}
	if !d.Enabled {
		return SIPTestResult{}, fmt.Errorf("%w: device is disabled", device.ErrInvalid)
	}
	transaction := newSIPTransaction()
	first, err := s.exchange(func(source string) string {
		return s.registerPacket(d, expires, transaction, "", "", source)
	})
	if err != nil {
		return SIPTestResult{}, err
	}
	...
	uri := s.uri(d)
	authorization := digestAuthorization(d, challenge, "REGISTER", uri)
	transaction.cseq++
	second, err := s.exchange(func(source string) string {
		return s.registerPacket(d, expires, transaction, first.Header("To"), authorization, source)
	})
	if err != nil {
		return SIPTestResult{}, err
	}
...
func (s *SIPSimulator) KeepAlive(ctx context.Context, accessID string) (SIPTestResult, error) {
	...
	response, err := s.exchange(func(source string) string {
		return s.keepAlivePacket(d, source)
	})
	if err != nil {
		return SIPTestResult{}, err
	}
...
func (s *SIPSimulator) Unregister(ctx context.Context, accessID string) (SIPTestResult, error) {
	...
	transaction := newSIPTransaction()
	response, err := s.exchange(func(source string) string {
		return s.registerPacket(d, 0, transaction, "", "", source)
	})
	if err != nil {
		return SIPTestResult{}, err
	}
...
func (s *SIPSimulator) uri(d device.Device) string {
	host := s.host
	if ip, err := resolveHost(s.host); err == nil && ip != "" {
		host = ip
	}
	return fmt.Sprintf("sip:%s@%s:%d", d.SIPRealm, host, s.port)
}
...
func digestAuthorization(d device.Device, challenge digestChallenge, method, uri string) string {
	cnonce := randomHex(8)
	ha2 := md5Hex(method + ":" + uri)
	response := md5Hex(strings.Join([]string{
		d.DigestHA1, challenge.Nonce, "00000001", cnonce, "auth", ha2,
	}, ":"))
	return fmt.Sprintf(
		`Digest username="%s", realm="%s", nonce="%s", uri="%s", response="%s", algorithm=%s, cnonce="%s", qop=auth, nc=00000001`,
		d.SIPUsername, challenge.Realm, challenge.Nonce, uri, response, challenge.Algorithm, cnonce)
}
...
func (s *SIPSimulator) registerPacket(d device.Device, expires int, transaction sipTransaction, toHeader, authorization, source string) string {
	uri := s.uri(d)
	from := fmt.Sprintf(`<sip:%s@%s>;tag=%s`, d.SIPUsername, d.SIPRealm, transaction.tag)
	to := fmt.Sprintf(`<sip:%s@%s>`, d.SIPUsername, d.SIPRealm)
	if toHeader != "" {
	...
	builder.WriteString("Contact: <sip:" + d.SIPUsername + "@" + source + ">\r\n")
...
func (s *SIPSimulator) keepAlivePacket(d device.Device, source string) string {
	callID := randomHex(8) + "@" + randomHex(4)
	branch := "z9hG4bK" + randomHex(12)
	tag := randomHex(8)
	body := fmt.Sprintf(
		...
		d.SIPUsername)
	var builder strings.Builder
	fmt.Fprintf(&builder, "MESSAGE sip:%s@%s SIP/2.0\r\n", d.SIPUsername, d.SIPRealm)
	builder.WriteString("Via: SIP/2.0/UDP " + source + ";branch=" + branch + ";rport\r\n")
	builder.WriteString("From: " + fmt.Sprintf(`<sip:%s@%s>;tag=%s`, d.SIPUsername, d.SIPRealm, tag) + "\r\n")
	builder.WriteString("To: " + fmt.Sprintf(`<sip:%s@%s>`, d.SIPUsername, d.SIPRealm) + "\r\n")
