...
const login = await api('POST', '/api/v1/auth/login', { body: { tenant: 'default', username: 'vision', password: OLD } })
...
const admin = (users.json ?? []).find((u) => u.username === 'vision')
...
const verify = await api('POST', '/api/v1/auth/login', { body: { tenant: 'default', username: 'vision', password: NEW } })
console.log('login with new password:', verify.status === 200 ? 'OK' : `FAIL ${verify.status}`)
const oldVerify = await api('POST', '/api/v1/auth/login', { body: { tenant: 'default', username: 'vision', password: OLD } })
